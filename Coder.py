...
def run_lenght(data, ha):
    ...
    for i in data:
        if i != 0 or zero_counter == 15:
            cat = category(i)
            ha_code = ha[(zero_counter, cat)]
            ac_help = convert(i, cat)
            rl.append(ha_code + ac_help)
            zero_counter = 0
        else:
            zero_counter += 1
    eob_code = ha[(0, 0)]
...
def encoding_blocks(data, chrominance=0):
    ...
    for block in d:
        ...
        DC_code = dc_ha[dc_category]
        DC_help = convert(block[0], dc_category)
        DC = DC_code + DC_help
    ...
    bitstream += '0' * pad_len
    byte_data = int(bitstream, 2).to_bytes(len(bitstream) // 8, 'big')

    header = (
            height.to_bytes(2, 'big') +
            width.to_bytes(2, 'big') +
            pad_len.to_bytes(1, 'big')
    )
...
def decoding_blocks(data, chrominance=0):
    ...
    pad_len = int.from_bytes(data[4:5], 'big')

    byte_data = data[5:]
    bitstream = bin(int.from_bytes(byte_data, 'big'))[2:]
    bitstream = bitstream.zfill(len(byte_data) * 8)
    ...
    dc_ha = {v: k for k, v in dc_ha.items()}
    ac_ha = {v: k for k, v in ac_ha.items()}
    ...
    while pos < len(bitstream):
        block = []
        word = ''
        while word not in dc_ha:
            word += bitstream[pos]
            pos += 1
        cat = dc_ha[word]
        DC = iconvert(bitstream[pos: pos + cat], cat)
        block.append(DC)

        while True:
            pos += cat
            word = ''
            while word not in ac_ha:
                word += bitstream[pos]
                pos += 1
            zeros, cat = ac_ha[word]
            if (zeros, cat) == (0, 0):
                ...
                block += [0] * zeros
                AC = iconvert(bitstream[pos: pos + cat], cat)
                block.append(AC)
        blocks.append(block)
